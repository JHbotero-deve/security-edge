...
export const Encabezado = () => {
  ...
  const { appName } = useConfiguracion();
      ...
      <div className="flex items-center gap-4 flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            <p className="text-sm font-black text-white italic leading-none">{user?.name || 'Operador'}</p>
            <p className="text-[9px] text-slate-500 mt-1 uppercase tracking-widest font-bold">Protocolo Activo</p>
          </div>

          <div className="group relative">
            <button className="w-10 h-10 bg-slate-900 rounded-full flex items-center justify-center border border-slate-800 hover:border-primary-500 transition-all overflow-hidden">
            ...
            <div className="absolute right-0 mt-2 w-56 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all py-3 translate-y-2 group-hover:translate-y-0 z-50">
              <div className="px-4 pb-3 border-b border-slate-800 mb-2">
                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mb-1">Identidad</p>
                <p className="text-xs text-white font-medium truncate">{user?.email}</p>
              </div>

              <button className="w-full text-left px-4 py-2 text-sm text-slate-400 hover:bg-slate-800 hover:text-white flex items-center gap-2 transition-colors">
