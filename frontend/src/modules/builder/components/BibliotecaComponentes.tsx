...
export const LoginGradient = ({ themeColor = '#3b82f6' }: { themeColor?: string }) => (
  <div
    ...
    style={{ background: `linear-gradient(160deg, ${themeColor} 0%, ${themeColor}cc 40%, #0f172a 100%)` }}
  >
    <div className="relative z-10">
      <div className="text-center mb-8">
        <div className="w-16 h-16 mx-auto mb-4 rounded-2xl flex items-center justify-center bg-white/15 backdrop-blur-sm">
          <ShieldCheck size={30} />
        </div>
        <h3 className="text-2xl font-black uppercase italic tracking-tighter">Bienvenido de Nuevo</h3>
        <p className="text-[10px] text-white/70 font-bold uppercase tracking-widest mt-1">Elige el color que representa tu marca</p>
      </div>
      <div className="space-y-4">
        <Entrada className="bg-white/10 border-white/20 text-white placeholder:text-white/50" label="Correo Electrónico" />
        <Entrada type="password" className="bg-white/10 border-white/20 text-white placeholder:text-white/50" label="Contraseña" />
        <button className="w-full py-3 rounded-xl bg-white font-black text-[11px] uppercase tracking-widest" style={{ color: themeColor }}>Ingresar</button>
      </div>
    </div>
  </div>
);
