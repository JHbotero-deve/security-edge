...
export const PaginaBuilder = () => {
  ...
  const loadProjectsList = useCallback(async () => {
    ...
    try {
      ...
      setSavedProjects(response.data.data.items);
    } catch (error) {
      setSavedProjects([]);
    }
  }, [isAuthenticated]);
  ...
  const saveProject = useCallback(async () => {
    if (!isAuthenticated) {
      alert('Para guardar tu proyecto necesitas una cuenta. Puedes seguir mirando y probando el constructor libremente, pero el guardado es solo para usuarios registrados.');
      navigate('/login');
      return;
    }
    ...
    try {
      if (currentProjectId) {
        const response = await api.put(`/builder/${currentProjectId}`, project);
        setSavedProjects(prev => prev.map(p => p.id === currentProjectId ? response.data.data : p));
      } else {
        const response = await api.post('/builder', project);
        setCurrentProjectId(response.data.data.id);
        setSavedProjects(prev => [response.data.data, ...prev]);
      }
      alert(`Proyecto "${projectName}" sincronizado con el servidor.`);
    } catch (error) {
      alert('No se pudo sincronizar. Verifica tu conexión o vuelve a iniciar sesión.');
    } finally {
      setIsSyncing(false);
    }
  }, [buildProjectPayload, currentProjectId, projectName, isAuthenticated, navigate]);

  const loadProject = useCallback((project: any) => {
    setProjectName(project.name);
    setThemeColor(project.themeColor || '#3b82f6');
    setCanvasBg(project.canvasBg || '#020617');
    setProjectPages(project.pages || { 'Principal': { items: [], type: 'marketing' } });
    setActivePage(Object.keys(project.pages || {})[0] || 'Principal');
  ...
  const deleteProject = useCallback(async (id: number) => {
    try {
      await api.delete(`/builder/${id}`);
      setSavedProjects(prev => prev.filter(p => p.id !== id));
      if (currentProjectId === id) setCurrentProjectId(null);
    } catch (error) {
      alert('No se pudo eliminar el proyecto (revisa tu conexión o sesión).');
    }
  }, [currentProjectId]);

  const startNewProject = useCallback(() => {
    setProjectName('Mi Proyecto Profesional');
    setThemeColor('#3b82f6');
    setCanvasBg('#020617');
    setProjectPages({ 'Principal': { items: [], type: 'marketing' } });
      ...
      <div className="relative flex min-h-screen -mt-8 -mx-8 bg-slate-950 font-sans overflow-hidden">
                ...
                <div className="space-y-4">
                  {!isAuthenticated && (
                    <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3 flex items-center gap-2">
                      ...
                      <p className="text-[9px] font-bold text-amber-500 uppercase tracking-wide leading-relaxed">
                        Modo vista previa — regístrate para guardar tus proyectos
                      </p>
                    </div>
                  )}
        ...
        <Modal isOpen={showProjectsList} onClose={() => setShowProjectsList(false)} title="Proyectos Guardados">
           {savedProjects.length === 0 ? (
             <p className="text-[11px] text-slate-500 font-bold uppercase tracking-widest text-center py-8">
                Todavía no has sincronizado ningún proyecto.
             </p>
           ) : (
             <div className="space-y-3">
                {savedProjects.map((project) => (
                  <div key={project.id ?? project.timestamp} className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl border border-slate-100">
                     <div>
                        <p className="text-sm font-black text-slate-900">{project.name}</p>
                        <p className="text-[9px] text-slate-500 uppercase tracking-widest">
                           {project.updatedAt ? new Date(project.updatedAt).toLocaleString() : 'Guardado local'}
                        </p>
                     </div>
                     <div className="flex gap-2">
                        <button onClick={() => loadProject(project)} className="px-4 py-2 bg-primary-600 text-white rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-primary-500">Abrir</button>
                        {project.id && (
                          <button onClick={() => deleteProject(project.id)} className="p-2 bg-red-50 text-red-600 rounded-xl hover:bg-red-100"><Trash2 size={14}/></button>
                        )}
                     </div>
                  </div>
                ))}
